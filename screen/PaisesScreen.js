...
const PaisesScreen = () => {
  ...
  useEffect(() => {
    ...
    axios.get(apiUrl)
      .then(response => {
        ...
        console.log('Dados dos países:', response.data);
      })
      .catch(error => {
  ...
  const handleSalvarEdicao = () => {
    const { id } = selectedPais;
    axios.put(`http://localhost:8080/pais/${id}`, {
      capital: editedCapital,
      nome: editedNome
    })
    .then(response => console.log('Dados atualizados (PUT):', response.data))
    .catch(error => console.error('Erro ao atualizar dados (PUT):', error));
  ...
  const handleExcluir = (id) => {
    axios.delete(`http://localhost:8080/pais/${id}`)
      .then(response => {
        console.log('País excluído com sucesso:', id);
      })
      .catch(error => {
